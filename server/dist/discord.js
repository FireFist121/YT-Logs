...
exports.startDiscordQueueWorker = startDiscordQueueWorker;
...
function buildEmbed(event) {
    ...
    if (event.proof && event.proof.length > 0) {
        ...
        if (proofText.length > 1024)
            proofText = proofText.substring(0, 1021) + '...';
...
async function sendDiscordWebhook(event) {
    ...
    try {
        await db_1.DiscordQueue.create({
...
async function startDiscordQueueWorker() {
...
async function rehydrateBufferFromDB() {
    try {
        const pending = await db_1.DiscordQueue.find({ status: 'pending' }).sort({ created_at: 1 });
        if (pending.length === 0)
            return;
        for (const item of pending) {
            const url = item.webhook_url;
            const embeds = item.payload?.embeds ?? [];
            if (!embedBuffer.has(url))
                embedBuffer.set(url, []);
...
const urlNextRetryTime = new Map();
/**
 * Core flush function.
 *
...
async function flushQueue() {
    ...
    for (const [webhookUrl, buffer] of embedBuffer.entries()) {
        ...
        try {
            ...
            const response = await axios_1.default.post(webhookUrl, { embeds: batch }, axiosOptions);
            ...
            if (status === 429) {
                ...
                if (header1)
                    retryAfterSec = parseFloat(header1);
                else if (header2)
                    retryAfterSec = parseFloat(header2);
                ...
                await db_1.DiscordQueue.updateMany({ webhook_url: webhookUrl, status: 'pending' }, {
                    $set: { next_retry_at: nextRetryDate },
                    $inc: { retries: 1 },
                });
                // If any item has exceeded MAX_RETRIES, mark as failed
                await db_1.DiscordQueue.updateMany({ webhook_url: webhookUrl, status: 'pending', retries: { $gte: MAX_RETRIES } }, { $set: { status: 'failed' } });
                console.warn(`[Discord] ⚠️  Rate limited! Batch of ${batch.length} restored to buffer. ` +
            ...
            if (status === 401 || status === 404) {
                ...
                await db_1.DiscordQueue.updateMany({ webhook_url: webhookUrl, status: 'pending' }, { $set: { status: 'failed' } });
            ...
            if (status >= 400) {
                ...
                const pendingRows = await db_1.DiscordQueue.find({ webhook_url: webhookUrl, status: 'pending' })
                    .sort({ created_at: 1 })
                    .limit(batch.length);
                const ids = pendingRows.map(r => r._id);
                if (ids.length > 0) {
                    await db_1.DiscordQueue.updateMany({ _id: { $in: ids } }, { $set: { status: 'sent' } });
                }
            }
        }
        catch (err) {
