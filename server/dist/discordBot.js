    ...
    client.on('interactionCreate', async (interaction) => {
        if (interaction.isCommand() && interaction.commandName === 'setup-panel') {
            ...
            const embed = new discord_js_1.EmbedBuilder()
                ...
                .setColor('#5865F2')
                .addFields({ name: '🟢 Turn ON Bot', value: 'Starts monitoring all configured channels.', inline: true }, { name: '🔴 Turn OFF Bot', value: 'Stops all active monitors.', inline: true }, { name: '📊 Status', value: 'Checks how many streams are currently being monitored.', inline: false })
        ...
        if (interaction.customId === 'bot_on') {
            try {
                ...
                const urlInput = new discord_js_1.TextInputBuilder()
                    ...
                    .setRequired(true);
                const firstActionRow = new discord_js_1.ActionRowBuilder().addComponents(urlInput);
        ...
        if (interaction.customId === 'bot_off') {
            try {
                ...
                for (const channel of channels) {
                    ...
                    await channel.save();
                    (0, monitor_1.stopMonitorByChannelId)(channel.channel_id);
                }
                ...
                if (interaction.deferred) {
                    ...
                    await interaction.reply({ content: `❌ An error occurred: ${err.message}`, flags: [discord_js_1.MessageFlags.Ephemeral] });
                }
            }
            return;
        }
        if (interaction.customId === 'bot_status') {
            try {
                await interaction.deferReply({ flags: [discord_js_1.MessageFlags.Ephemeral] });
                const active = (0, monitor_1.getActiveMonitors)();
                const channels = await db_1.WatchedChannel.find({});
                const autoMonitoredCount = channels.filter(c => c.auto_monitor).length;
                const embed = new discord_js_1.EmbedBuilder()
                    .setTitle('📊 System Status')
                    .setColor('#2b2d31')
                    .addFields({ name: 'Total Configured Channels', value: `${channels.length}`, inline: true }, { name: 'Auto-Monitor ON', value: `${autoMonitoredCount}`, inline: true }, { name: 'Currently Live & Monitored', value: `${active.length}`, inline: true });
    ...
    client.on('interactionCreate', async (interaction) => {
        ...
        if (interaction.customId === 'modal_start_monitor') {
            ...
            try {
                ...
                await db_1.WatchedChannel.findOneAndUpdate({ channel_id: channelId }, {
                    $set: {
                        is_live: true,
