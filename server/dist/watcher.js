...
exports.startChannelWatcher = startChannelWatcher;
...
async function checkAll() {
    try {
        const watchedChannels = await db_1.WatchedChannel.find({});
        if (!watchedChannels)
            return;
        for (const channel of watchedChannels) {
            try {
                ...
                await db_1.WatchedChannel.updateOne({ channel_id: channel.channel_id }, {
                    $set: {
                ...
                if (liveStatus.isLive && liveStatus.liveChatId) {
                    (0, monitor_1.startMonitor)(channel.channel_id, liveStatus.liveChatId, liveStatus.videoId || undefined);
                }
...
async function checkChannelLiveStatus(channelId) {
    ...
    try {
        const res = await youtube_1.youtube.liveBroadcasts.list({
            part: ['snippet'],
            broadcastStatus: 'active',
            broadcastType: 'all',
        });
        if (res.data.items && res.data.items.length > 0) {
            const activeBroadcast = res.data.items.find((b) => b.snippet.channelId === channelId);
            if (activeBroadcast) {
                // Find live chat ID
    ...
    // 2. SEARCH.LIST
    try {
        const res = await youtube_1.youtube.search.list({
            part: ['snippet'],
            channelId,
            eventType: 'live',
            type: ['video'],
